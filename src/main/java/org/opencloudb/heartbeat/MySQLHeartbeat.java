...
public class MySQLHeartbeat extends DBHeartbeat {
	...
	public static final Logger log = LoggerFactory.getLogger(MySQLHeartbeat.class);
	...
	private void setError(MySQLDetector detector) {
		...
		if (++errorCount < this.maxRetryCount) {
            if (detector != null && !detector.isQuit()) {
	...
	private void switchSourceIfNeed(String reason) {
		...
		if (switchType == DataHostConfig.NOT_SWITCH_DS) {
			log.debug("not switch datasource, for switchType is {}", DataHostConfig.NOT_SWITCH_DS);
			return;
		}
						...
						if (theSourceHBStatus == DBHeartbeat.OK_STATUS) {
							if (switchType == DataHostConfig.SYN_STATUS_SWITCH_DS) {
								if (Integer.valueOf(0).equals( theSourceHB.getSlaveBehindMaster())) {
									log.info("try to switch dataSource ,slave is synchronized to master {}", theSource.getConfig());
									pool.switchSource(nextId, true, reason);
									break;
								} else {
									log.warn("ignored dataSource, sslave is not  synchronized to master, " +
											"slave behind master: {} {}", theSourceHB.getSlaveBehindMaster(), theSource.getConfig());
								}
							} else {
								// normal switch
								log.info("try to switch dataSource ,not checked slave synchronize status {}", theSource.getConfig());
								pool.switchSource(nextId, true, reason);
                                break;
							}
